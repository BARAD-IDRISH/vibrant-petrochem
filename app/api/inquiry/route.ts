  ...
  try {
    ...
    if (!fullName || !email || !phone) {
      return NextResponse.json(
        { error: 'Missing required contact information (Full Name, Email, Phone).' },
        { status: 400 }
      );
    }

    // SMTP Configuration with Zero-.env Default Transporter
    const user = process.env.SMTP_USER || process.env.EMAIL_USER || 'webbhe1313@gmail.com';
    const pass = process.env.SMTP_PASS || process.env.EMAIL_PASS || 'ijyr vhph fipi hnwd';
    ...
    if (process.env.SMTP_HOST) {
      ...
      // Default Gmail SMTP Transporter (No .env file required!)
      transporter = nodemailer.createTransport({
        service: 'gmail',
        auth: {
          user,
    ...
    await transporter.sendMail({
      ...
      to: recipient,
      replyTo: email, // Click reply in inbox to directly reply to the client
